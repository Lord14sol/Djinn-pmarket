...
declare_id!("CMA2yM8ZUEiJ2t9cYNYShGaH2TbvcxPh2ZKbotm8wUHL"); 
...
const VIRTUAL_SOL_INIT: u64 = 10_000_000_000; // 10 SOL (As requested for Chaos/Degen mode)
const ENDGAME_THRESHOLD_BPS: u64 = 9_500; // 0.95 SOL price
const WHALE_LIMIT: u64 = 50_000_000_000_000_000; // 50M Tokens (5% of 1B Supply)
...
pub mod djinn_market {
    ...
    pub fn create_market(
        ...
        require!(initial_prob_bps > 0 && initial_prob_bps < 10000, DjinnError::InvalidAmount);
        ...
        market.last_trade_slot = 0;
        market.last_trader = Pubkey::default();

        // VIRTUAL AMM INIT (x * y = k)
        // DEGEN MODE: 1B SUPPLY, 10 SOL LIQUIDITY.
        // Price = 10 / 1B = 10^-8 SOL (Cheap entry)
        
        let target_shares: u128 = 1_000_000_000 * 1_000_000_000; // 1 Billion Tokens (Atomic)

        market.virtual_sol_reserves = VIRTUAL_SOL_INIT;
        market.virtual_share_reserves = target_shares as u64;

        msg!("Market Created. Degen Mode: 1B Supply. Virtual Liq: {} SOL", VIRTUAL_SOL_INIT);
